<?php
	// A completer
	if(isset($_GET['style'])){
    setcookie("style", $_GET['style'], time() + 300);
    } else {
        if(!isset($_COOKIE['style'])){
            setcookie("style", "style1.css", time() +300);
        } else {
	...
	// Si oui : créez/modifiez le cookie
	
	// Si non : soit le cookie existe et on a le style demandé
	//          soit le cookie n'existe pas et il faut le créer avec une valeur par défaut 
...
<head>
  <meta charset="utf-8">
   <title>Testons les cookies</title>
    <?php
    echo "<link rel='stylesheet' type='text/css' href='".htmlentities($_COOKIE['style'])."'/>";
